...
{
    [TestFixture]
    public class EmployeeTokensControllerTests
    {
        IEmployeeTokensService _tokensService;

        [SetUp]
        public void Set_TokensService()
        {
            _tokensService = A.Fake<IEmployeeTokensService>();
        }
        ...
        public async Task Token_Should_Create_token_typeof_TokenDTO_And_Return_Status_Code_200()
        {
            ...
            A.CallTo(() => _tokensService.CreateTokenAsync(A<LoginRequest>._))
                .Returns(new TokenDTO { Jwt = "jwt_token", RefreshToken = "refreh_token" });

            var employeeTokensController = new EmployeeTokensController(_tokensService);

            //Act
            var actionResult = await employeeTokensController.Token(request);
        ...
        public async Task RefreshToken_Should_Create_token_typeof_TokenDTO_And_Return_Status_Code_200()
        {
            ...
            //Act
            var actionResult = await employeeTokensController.RefreshToken(request);

...
{
    [TestFixture]
    public class RecruiterTokensControllerTests
    {
        ITokensService<RecruiterDTO> _tokensService;

        [SetUp]
        public void Set_TokensService()
        {
            _tokensService = A.Fake<ITokensService<RecruiterDTO>>();
        }
        [Test]
        public async Task Token_Should_Create_token_typeof_TokenDTO_And_Return_Status_Code_200()
        {
            ...
            var recruiterTokensController = new RecruiterTokensController(_tokensService);

            //Act
            var actionResult = await recruiterTokensController.Token(request);
        ...
        public async Task RefreshToken_Should_Create_token_typeof_TokenDTO_And_Return_Status_Code_200()
        {
            ...
            //Act
            var actionResult = await recruiterTokensController.RefreshToken(request);

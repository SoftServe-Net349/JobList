...
namespace JobList.Common.Validators
{
    public class EmployeeUpdateValidator : AbstractValidator<EmployeeUpdateRequest>
    {
        ...
        {
            RuleFor(x => x.FirstName).NotEmpty().Length(2, 50).WithMessage("Please specify a valid First Name.");
            ...
            RuleFor(x => x.Email).NotEmpty().Length(4, 254).WithMessage("Please specify a valid Email.");

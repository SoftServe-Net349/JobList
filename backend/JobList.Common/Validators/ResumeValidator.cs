...
namespace JobList.Common.Validators
{
    public class ResumeValidator : AbstractValidator<ResumeRequest>
    {
        public ResumeValidator()
        {
            ...
            RuleFor(x => x.FamilyState).Length(2, 20).WithMessage("Please specify a valid FamilyState. Max length: 20, Min length: 2");
            RuleFor(x => x.Linkedin).Length(2, 200).WithMessage("Please specify a valid Linkedin. Max length: 20, Min length: 2");
            RuleFor(x => x.Github).Length(2, 200).WithMessage("Please specify a valid Github. Max length: 20, Min length: 2");
            RuleFor(x => x.Skype).Length(2, 200).WithMessage("Please specify a valid Skype. Max length: 20, Min length: 2");
            RuleFor(x => x.Instagram).Length(2, 200).WithMessage("Please specify a valid Instagram. Max length: 20, Min length: 2");
            RuleFor(x => x.Facebook).Length(2, 200).WithMessage("Please specify a valid Facebook. Max length: 20, Min length: 2");
